...
(function (root, factory) {
  ...
  function createTerrain(seed) {
    ...
    for (let y = 0; y < SIZE; y++) {
      for (let x = 0; x < SIZE; x++) {
        ...
        if (value > max) { max = value; peak = { x: x, y: y }; }
      }
    }
  ...
  function observe(terrain, observations, x, y) {
    ...
    if (observations.some(function (point) { return point.x === x && point.y === y; })) throw new Error('You have already sampled that point. Choose somewhere new.');
  ...
  return Object.freeze({ VERSION: VERSION, BUDGET: BUDGET, validPoint: validPoint, createTerrain: createTerrain, observe: observe, randomBaseline: randomBaseline });
});
