(function () {
  ...
  const $ = function (id) { return document.getElementById('th-' + id); };
  ...
  function isLocked() { return busy || Boolean(liveState && liveState.closed) || Boolean(api && round.mode === 'daily' && !liveState); }
  function setBusy(value) {
    busy = value;
    $('x').disabled = value;
    $('y').disabled = value;
    ['daily', 'practice', 'new-practice', 'new-day-button'].forEach(function (id) { $(id).disabled = value; });
  ...
  async function openSharedDaily() {
    ...
    try {
      if (!registered) { fillIdentity(await api.checkIn()); registered = true; }
      activateShared(await api.start());
      status(liveState.complete ? 'Score saved to today’s leaderboard. Terrain opens after midnight UTC.' : 'Connected. Your daily probes and score are saved by the server.');
      pendingProbe = null;
      await refreshCommunity();
  ...
  async function probeShared(x, y) {
    ...
    try {
      ...
      pendingProbe = null;
      activateShared(next);
      status(next.complete ? 'Five probes complete. Your score is on the shared leaderboard.' : 'Probe recorded. ' + next.remaining + ' probes remain.');
      if (next.complete) await refreshCommunity();
      return state();
    } catch (error) {
      if (error.status) {
        pendingProbe = null;
        try { activateShared(await api.get(round.id)); } catch (_) { /* Preserve last known state. */ }
      }
  ...
  async function refreshCommunity() {
    ...
    try {
      ...
      tableRows($('leaderboard'), board.entries.map(function (entry) {
        return [entry.name, [entry.agent, entry.model].filter(Boolean).join(' / ') || (entry.kind === 'human' ? 'Human' : 'Unspecified'), formatSignal(entry.score), relativeTime(entry.completedAt)];
      }), 'No completed searches today. The first score is yours to set.');
      tableRows($('activity'), activity.recent.map(function (entry) {
        return [entry.name, [entry.agent, entry.model].filter(Boolean).join(' / ') || 'Unspecified agent', entry.checkins, relativeTime(entry.lastSeen)];
      }), 'No agents have checked in yet.');
      $('activity-counts').textContent = activity.agentIdentities + ' agent identities · ' + activity.agentCheckinsToday + ' agent check-ins today · ' + activity.checkins + ' total check-ins · ' + activity.completedGames + ' completed games';
      $('agent-types').textContent = activity.types.map(function (entry) { return entry.agent + ': ' + entry.checkins + (entry.checkins === 1 ? ' check-in' : ' check-ins'); }).join(' · ');
      $('live-status').textContent = board.day + ' UTC · updated just now';
    } catch (_) { $('live-status').textContent = 'Updates unavailable. Retrying shortly.'; }
  ...
  function mapPoint(point) { return { x: 40 + point.x * 5.6, y: 600 - point.y * 5.6 }; }
  function setVisible(node, visible) {
    if (visible) node.removeAttribute('hidden');
    else node.setAttribute('hidden', '');
  }
  function formatSignal(signal) { return signal.toFixed(1); }
  ...
  function validTimestamp(value) { return Number.isFinite(value) && value >= 0 && value <= 8640000000000000; }
  ...
  function save() {
    if (storageAvailable) {
      ...
      latest.rounds.forEach(function (item) {
        const existing = store.rounds.findIndex(function (candidate) { return candidate.id === item.id; });
    ...
    store.rounds = store.rounds.filter(function (item) { return item.id !== round.id; }).concat([round]).slice(-40);
  ...
  function activate(next) {
    ...
    observations = round.points.map(function (point) { return { x: point.x, y: point.y, signal: terrain.signal(point.x, point.y), at: point.at }; });
    selection = null;
    result = null;
    replayStep = engine.BUDGET;
    $('x').value = '';
    ...
    save();
    render();
    status(observations.length === engine.BUDGET ? 'All five probes are in. Trace your search on the revealed map.' : observations.length ? 'Expedition resumed. Your previous observations are saved below.' : 'Choose a point on the map, or enter coordinates.');
  }

  function openDaily() {
    if (api) return openSharedDaily();
    if (storageAvailable) store = readStore();
    activate(store.rounds.find(function (item) { return item.id === 'daily:' + utcDay(); }) || makeRound('daily'));
  }

  function bestObservation() {
    return observations.reduce(function (best, point) { return !best || point.signal > best.signal ? point : best; }, null);
  }
  ...
  function state() {
    ...
    return {
      ...
      observations: observations.map(function (point) { return Object.assign({}, point); }),
  ...
  function probe(x, y) {
    ...
    if (storageAvailable) {
      const latest = readStore().rounds.find(function (item) { return item.id === round.id; });
      if (latest && latest.points.length > round.points.length) activate(latest);
    }
    let point;
    try { point = engine.observe(terrain, observations, x, y); }
    ...
    status('Probe ' + observations.length + ': signal ' + formatSignal(point.signal) + ' at (' + x + ', ' + y + '). ' + (remaining ? remaining + (remaining === 1 ? ' probe remains.' : ' probes remain.') : 'Expedition complete. The full terrain is now revealed.'));
    return state();
  }
  ...
  function drawProbes() {
    ...
    if (completed && visible.length > 1) {
      const path = visible.map(function (point, index) {
    ...
    visible.forEach(function (point, index) {
      ...
      group.appendChild(svg('text', { y: .5, class: 'th-pin-number' }, index + 1));
      const alignRight = point.x > 83;
      group.appendChild(svg('text', { x: alignRight ? -15 : 15, y: point.y < 5 ? -12 : 19, 'text-anchor': alignRight ? 'end' : 'start', class: 'th-pin-signal' }, formatSignal(point.signal)));
  ...
  function revealTerrain() {
    ...
    if (context) {
      ...
      $('terrain').appendChild(svg('image', { x: 40, y: 40, width: 560, height: 560, href: canvas.toDataURL(), class: 'th-terrain-image', preserveAspectRatio: 'none' }));
    }
    ...
    for (let level = 10; level < 100; level += 10) {
      ...
      for (let y = 0; y < 100; y += 2) {
        for (let x = 0; x < 100; x += 2) {
          const corners = [[x, y], [x + 2, y], [x + 2, y + 2], [x, y + 2]].map(function (p) { return { x: p[0], y: p[1], value: terrain.signal(p[0], p[1]) }; });
          [[corners[0], corners[1], corners[2]], [corners[0], corners[2], corners[3]]].forEach(function (triangle) {
            const intersections = [];
            ...
            if (intersections.length === 2) segments.push('M' + intersections[0].x.toFixed(1) + ' ' + intersections[0].y.toFixed(1) + 'L' + intersections[1].x.toFixed(1) + ' ' + intersections[1].y.toFixed(1));
          });
        }
      }
    ...
    group.appendChild(svg('text', { x: terrain.peak.x > 75 ? -16 : 16, y: -12, 'text-anchor': terrain.peak.x > 75 ? 'end' : 'start', class: 'th-peak-label' }, 'Maximum: 100'));
  ...
  function render() {
    ...
    $('probe-dots').querySelectorAll('i').forEach(function (dot, index) { dot.classList.toggle('is-used', index < observations.length); });
    ...
    $('map').setAttribute('aria-label', revealed ? 'Revealed terrain and your probe locations. Use the replay slider below to retrace your search.' : 'Survey map. Use arrow keys to choose coordinates and Enter to probe.');
    $('map-label').textContent = revealed ? 'Revealed landscape' : completed ? 'Your completed search' : 'Search space';
    $('map-help').textContent = revealed ? '✦ marks the highest point.' : completed ? 'Terrain opens after midnight UTC.' : 'Click to select. Probe to discover.';
    ...
    if (completed) {
      if (!result) result = { score: best.signal, peak: Object.assign({}, terrain.peak), randomBaseline: engine.randomBaseline(terrain, best.signal) };
      if (revealed) revealTerrain();
      $('result-title').textContent = result.score >= 99.5 ? 'Near the maximum' : result.score >= 75 ? 'Strong result' : result.score >= 40 ? 'Search complete' : 'Search complete';
      if (result.score === 100) $('result-title').textContent = 'Maximum found';
      $('result-copy').textContent = 'Your best signal was ' + formatSignal(result.score) + (revealed ? '. The highest point is at (' + terrain.peak.x + ', ' + terrain.peak.y + '), with a signal of 100.' : '. Saved to today’s leaderboard. The full terrain opens after midnight UTC.');
      $('baseline').textContent = result.randomBaseline ? 'Better than ' + result.randomBaseline.percentile + '% of 512 simulated five-probe random searches on this map. A little context, not a model benchmark.' : '';
    }
    drawProbes();
    renderHistory();
    checkNewDay();
  ...
  function renderHistory() {
    const completed = store.rounds.filter(function (item) { return item.points.length === engine.BUDGET; }).sort(function (a, b) { return b.updatedAt - a.updatedAt; });
    $('journal-total').textContent = completed.length ? completed.length + (completed.length === 1 ? ' saved game' : ' saved games') + ' · local only' : 'Saved in this browser';
    ...
    completed.slice(0, 6).forEach(function (item) {
      ...
      const score = Math.max.apply(null, item.points.map(function (point) { return itemTerrain.signal(point.x, point.y); }));
  ...
  function checkNewDay() { $('new-day').hidden = round.mode !== 'daily' || round.date === utcDay(); }
  async function refreshExpiredTerrain() {
    ...
    try {
      ...
      if (!busy && liveState && round.id === id) { activateShared(next); status('This daily challenge has closed. Its full terrain is now available.'); }
  ...
  $('probe-form').addEventListener('submit', async function (event) {
    ...
    if (!selection) return;
    try { await probe(selection.x, selection.y); } catch (error) { status(error.message, true); }
  });
  ['x', 'y'].forEach(function (id) { $(id).addEventListener('input', updateSelection); });
  ...
  $('map').addEventListener('keydown', async function (event) {
    ...
    if (keys[event.key]) {
      ...
      else { try { await probe(selection.x, selection.y); } catch (error) { status(error.message, true); } }
    }
  });
  ...
  $('practice').addEventListener('click', function () {
    ...
    const existing = store.rounds.slice().reverse().find(function (item) { return item.mode === 'practice' && item.points.length < engine.BUDGET; });
    activate(existing || makeRound('practice'));
  });
  $('new-practice').addEventListener('click', function () { activate(makeRound('practice')); });
  $('replay-step').addEventListener('input', function () { replayStep = Number($('replay-step').value); drawProbes(); });
  ...
  window.addEventListener('storage', function (event) {
    ...
    store = readStore();
    const updated = store.rounds.find(function (item) { return item.id === round.id; });
    if (updated && updated.points.length > round.points.length) activate(updated);
    else renderHistory();
  });
  document.addEventListener('visibilitychange', function () { if (!document.hidden) { checkNewDay(); renderHistory(); } });
  window.setInterval(function () { checkNewDay(); renderHistory(); }, 60000);

  const currentDay = utcDay();
  const savedActive = store.rounds.find(function (item) { return item.id === store.activeId; });
  const initial = savedActive && (savedActive.mode === 'practice' || savedActive.date === currentDay) ? savedActive : store.rounds.find(function (item) { return item.id === 'daily:' + currentDay; }) || makeRound('daily');
  activate(initial);
  ...
  if (api) {
    ...
    window.setInterval(function () { if (!document.hidden) { refreshCommunity(); refreshExpiredTerrain(); } }, 30000);
    document.addEventListener('visibilitychange', function () { if (!document.hidden) { refreshCommunity(); refreshExpiredTerrain(); } });
  }
