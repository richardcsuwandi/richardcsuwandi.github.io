(function () {
  ...
  class TreasureHuntAPI {
    constructor(base) {
      ...
      try { this.token = localStorage.getItem(KEY); } catch (_) { /* In-memory access still works. */ }
    }
    ...
    async checkIn(identity) {
      let data;
      try { data = await this.request('/v1/visits', { ...identity, eventId: this.eventId }, true); }
      catch (error) {
      ...
      if (data.token) {
        ...
        try { localStorage.setItem(KEY, data.token); } catch (_) { /* Keep the token in memory. */ }
      }
      this.visitor = data.visitor;
      return data.visitor;
    }
    start() { return this.request('/v1/runs', {}, true); }
    get(id) { return this.request('/v1/runs/' + id); }
    probe(id, x, y, requestId) { return this.request('/v1/runs/' + id + '/probes', { x, y, requestId }, true); }
    leaderboard() { return this.request('/v1/leaderboard'); }
    activity() { return this.request('/v1/activity'); }
  }
