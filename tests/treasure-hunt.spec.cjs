...
test('agent interface validates probes, hides the result until completion, and returns defensive copies', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    const messages = [];
    for (const [x, y] of [[-1, 2], [101, 1], [1.2, 5], ['2', 4]]) {
      try { window.treasureHunt.probe(x, y); } catch (error) { messages.push(error.message); }
    }
    window.treasureHunt.probe(0, 100);
    try { window.treasureHunt.probe(0, 100); } catch (error) { messages.push(error.message); }
...
test('denied storage still allows a complete game', async ({ page }) => {
  ...
  await page.evaluate(() => { for (let x = 0; x < 5; x++) window.treasureHunt.probe(x * 20, 50); });
  await expect(page.locator('#th-result')).toBeVisible();
});
