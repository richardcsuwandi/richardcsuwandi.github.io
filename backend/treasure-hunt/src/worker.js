...
function detectedAgent(userAgent) {
  for (const [pattern, name] of [
    [/\bOAI-SearchBot\b/i, 'OAI-SearchBot'], [/\bGPTBot\b/i, 'GPTBot'], [/\bChatGPT-User\b/i, 'ChatGPT-User'],
    [/\bClaudeBot\b/i, 'ClaudeBot'], [/\bClaude-SearchBot\b/i, 'Claude-SearchBot'], [/\bClaude-User\b/i, 'Claude-User'],
    [/\bPerplexityBot\b/i, 'PerplexityBot'], [/\bPerplexity-User\b/i, 'Perplexity-User'],
    [/\bClaude-Code\b|\bclaude-cli\b/i, 'Claude Code'], [/\bCodex\b/i, 'Codex']
...
function label(value, max = 60) {
  ...
  if (typeof value !== 'string' || value.length > max || /[\x00-\x1f\x7f]/.test(value)) throw new HttpError(400, 'Identity fields must be short, plain text strings.');
  return value.trim() || null;
}
function validId(value) { return typeof value === 'string' && /^[a-zA-Z0-9_-]{8,100}$/.test(value); }
...
function identity(body, request, existing) {
  ...
  if (body.kind !== undefined && !['human', 'agent', 'unknown'].includes(body.kind)) throw new HttpError(400, 'kind must be human, agent, or unknown.');
  if (!hasIdentity && existing) return {
    name: existing.display_name, kind: existing.kind, agent: existing.agent, model: existing.model,
    detected: detected || existing.detected_agent, source: existing.identity_source
  };
  const kind = reportedAgent || model || detected ? 'agent' : (body.kind || 'unknown');
  return { name: name || reportedAgent || detected || existing?.display_name || 'Anonymous visitor', kind,
    agent: reportedAgent || detected, model, detected,
    source: reportedAgent || model || body.kind === 'agent' ? 'self-reported' : detected ? 'user-agent (unverified)' : body.kind === 'human' ? 'self-reported' : 'unknown' };
}
...
async function readBody(request) {
  ...
  while (true) {
    ...
    if (length > MAX_BODY) { await reader.cancel(); throw new HttpError(413, 'Request body is too large.'); }
    chunks.push(value);
  }
  ...
  let body;
  try { body = JSON.parse(new TextDecoder().decode(bytes)); } catch { throw new HttpError(400, 'Invalid JSON.'); }
...
async function authenticate(request, env, optional = false) {
  ...
  if (!header || !/^Bearer [a-zA-Z0-9-]{40,100}$/.test(header)) throw new HttpError(401, 'Register at POST /v1/visits, then send Authorization: Bearer <token>.');
...
async function rateLimit(request, env, category, max) {
  ...
  const row = await env.DB.prepare('INSERT INTO rate_buckets (key, count, expires_at) VALUES (?, 1, ?) ON CONFLICT(key) DO UPDATE SET count = count + 1 RETURNING count')
...
async function publicState(env, run) {
  ...
  return {
    version: 1, edition: 'shared-daily', round: { id: run.id, mode: 'daily', date: run.day },
    domain: { x: [0, 100], y: [0, 100], integersOnly: true }, budget: 5,
    remaining: 5 - run.probes_used, observations, best, complete, closed,
    revealAt: new Date(revealAt).toISOString(), terrainSeed: closed ? seed : null,
    result: complete || closed ? { score: best?.signal || 0, peak: closed ? terrain.peak : null,
      randomBaseline: complete ? engine.randomBaseline(terrain, best.signal) : null } : null
  };
}
...
async function checkIn(request, env) {
  ...
  if (!visitor) {
    ...
    await env.DB.prepare('INSERT INTO visitors (id, token_hash, display_name, kind, agent, model, detected_agent, identity_source, first_seen, last_seen, checkins) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)')
      .bind(id, await sha256(issuedToken), details.name, details.kind, details.agent, details.model, details.detected, details.source, now, now).run();
    visitor = { id };
  }
  // A repeated page-load event does not inflate the count. Both writes commit together.
  await env.DB.batch([
    env.DB.prepare('INSERT OR IGNORE INTO checkins (id, visitor_id, kind, agent, at) VALUES (?, ?, ?, ?, ?)')
      .bind(visitor.id + ':' + eventId, visitor.id, details.kind, details.agent, now),
    env.DB.prepare('UPDATE visitors SET checkins = checkins + changes(), display_name = ?, kind = ?, agent = ?, model = ?, detected_agent = ?, identity_source = ?, last_seen = ? WHERE id = ?')
  ...
  return { visitor: { id: visitor.id, name: details.name, kind: details.kind, agent: details.agent, model: details.model, identitySource: details.source }, ...(issuedToken ? { token: issuedToken } : {}) };
}
...
async function startRun(request, env) {
  ...
  await env.DB.prepare('INSERT OR IGNORE INTO runs (id, visitor_id, day, started_at, updated_at) VALUES (?, ?, ?, ?, ?)')
    .bind(crypto.randomUUID(), visitor.id, day, now, now).run();
...
async function submitProbe(request, env, id) {
  ...
  let point;
  try { point = engine.observe(terrain, points, body.x, body.y); }
  catch (error) { throw new HttpError(error instanceof RangeError ? 400 : 409, error.message); }
  const now = Date.now();
  points.push({ ...point, at: now, requestId: body.requestId });
  // Compare-and-swap makes the budget safe under concurrent submissions.
  const write = await env.DB.prepare('UPDATE runs SET observations = ?, probes_used = ?, best_score = ?, updated_at = ?, completed_at = ? WHERE id = ? AND visitor_id = ? AND probes_used = ?')
    .bind(JSON.stringify(points), points.length, Math.max(run.best_score, point.signal), now, points.length === 5 ? now : null, run.id, visitor.id, run.probes_used).run();
...
async function leaderboard(env, url) {
  ...
  const { results } = await env.DB.prepare(`SELECT r.id, v.display_name AS name, v.kind, v.agent, v.model, v.identity_source AS identitySource,
    r.best_score AS score, r.completed_at AS completedAt FROM runs r JOIN visitors v ON v.id = r.visitor_id
    WHERE r.day = ? AND r.probes_used = 5 ORDER BY r.best_score DESC, r.completed_at ASC LIMIT 50`).bind(day).all();
  return { day, entries: results, identityNotice: 'Agent and model names are self-reported or inferred from an unverified user-agent. One entry per visitor token per day.' };
}
...
async function activity(env) {
  ...
  const [totals, recent, kinds, today] = await env.DB.batch([
    ...
    env.DB.prepare(`SELECT COUNT(*) AS checkinsToday, COALESCE(SUM(CASE WHEN kind = 'agent' THEN 1 ELSE 0 END),0) AS agentCheckinsToday FROM checkins WHERE at >= ?`).bind(midnight)
  ]);
  return { ...totals.results[0], ...today.results[0], recent: recent.results, types: kinds.results,
    coverage: 'Browser check-ins and explicit agent API check-ins only. Plain HTML requests to GitHub Pages are not observable. Identities are visitor tokens, not verified unique agents.' };
}

function rules(env, url) {
  const base = url.origin;
  return {
    game: 'Black-box treasure hunt', website: env.SITE_ORIGIN + (env.SITE_PATH || '/agents/'), api: base,
    instructions: [
...
export default {
  async fetch(request, env) {
    ...
    if (env.LOCAL_DEV === 'true') { allowed.add('http://127.0.0.1:4017'); allowed.add('http://localhost:4000'); allowed.add('http://127.0.0.1:4000'); }
