...
function fixture() {
  ...
  const env = {
    GAME_SECRET: "test-secret-is-never-used-in-production",
...
test("concurrent probes cannot restore or overspend the budget", async () => {
  ...
  const attempts = await Promise.all(
    Array.from({ length: 10 }, (_, i) => request(path + "/probes", { x: i, y: i, requestId: "concurrent-" + i }, v.token))
  );
...
test("origin policy, malformed input and throttling", async () => {
  ...
  assert.equal(
    (await request("/v1/activity", undefined, undefined, { Origin: env.SITE_ORIGIN })).headers.get("Access-Control-Allow-Origin"),
    env.SITE_ORIGIN
  );
